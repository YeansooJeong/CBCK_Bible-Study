...
function ProjectsPage() {
  ...
  return <StudentShell><main className="management-shell">
    ...
    {!sharedOnly && <section className="management-card"><h2>새 프로젝트 만들기</h2>{error && <div className="notice error">{error}</div>}<form className="form-row" onSubmit={handleCreate}><input className="field" placeholder="프로젝트 제목 (예: 창세기 1강)" value={title} onChange={(e) => setTitle(e.target.value)} required/><select className="field" value={shareScope} onChange={(e) => setShareScope(e.target.value as ShareScope)}><option value="private">나만 보기</option><option value="all">전체 공유</option></select><button className="primary-button" type="submit"><Icon name="plus"/> 만들기</button></form></section>}
