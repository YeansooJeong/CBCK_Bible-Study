...
export async function createSessionToken(userId: string, secret: string, ttlSeconds = 60 * 60 * 24 * 7): Promise<string> {
  const payload: SessionPayload = { sub: userId, exp: Math.floor(Date.now() / 1000) + ttlSeconds }
